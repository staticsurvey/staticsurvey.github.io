...
const trialsFile = "./data/experiments"
...
function getTrialFileFullName() {

		var url = new URL(window.location);
		var id = url
				.searchParams
				.get("id");
		var file = `${trialsFile}_${id}.csv`
		console.log(file)
		return file;
		// Get Trails
}
...
function nextTrial() {
		...
		if (currentTrial <= numTrials) {
				var trialID = trialsData[currentTrial]["Trial No"];
				var menuType = trialsData[currentTrial]['Menu Type'];
				var menuDepth = trialsData[currentTrial]['Menu Depth'];
				var mouseType = trialsData[currentTrial]['Mouse Type'];
				var targetItem = trialsData[currentTrial]['Target Item'];

				document
						.getElementById("trialNumber")
						.innerHTML = String(currentTrial) + "/" + String(numTrials);
				document
						.getElementById("menuType")
						.innerHTML = menuType;
				document
						.getElementById("menuDepth")
						.innerHTML = menuDepth;
				document
						.getElementById("mouseType")
						.innerHTML = mouseType;
				document
						.getElementById("targetItem")
						.innerHTML = targetItem;
				document
...
function initializeMarkingMenu() {
		...
		if (radialMenuContainer != null) {
				radialMenuContainer
						.parentNode
						.removeChild(radialMenuContainer);
		}
		...
		if (markingMenuContainer == null) {
				interactionContainer.innerHTML += "<div id=\"marking-menu-container\" style=\"height:100%;width:100%\" onmousedown=" +
								"\"markingMenuOnMouseDown()\" oncontextmenu=\"preventRightClick(event)\"></div>";
		}
}

// Formats csv menu data in the structure accepted by radial menu Assumes menu
// csv is sorted by Id and Parent both Ascending
...
function initializeRadialMenu() {
		...
		var markingMenuContainer = document.getElementById('marking-menu-container');
		if (markingMenuContainer != null) {
				markingMenuContainer
						.parentNode
						.removeChild(markingMenuContainer);
		}
		...
		var radialMenuContainer = document.getElementById('radial-menu-container');
		if (radialMenuContainer == null) {
				interactionContainer.innerHTML += "<div id=\"radial-menu-container\" style=\"height:100%;width:100%\" oncontextmenu" +
								"=\"toggleRadialMenu(event)\"></div>";
		}
...
function createRadialMenu(radialMenuL) {
		...
		if (radialMenuSvgElement != null) {
				radialMenuSvgElement
						.parentNode
						.removeChild(radialMenuSvgElement);
		}
		radialMenuSvg = d3
...
function formatRadialMenuData(data) {
		...
		return {'_children': menuItemsList};
